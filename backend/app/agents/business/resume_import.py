...
IMPORT_SYSTEM_PROMPT = (
    ...
    "2. type 字段必须且只能使用以下英文取值之一（与前端编辑器严格对齐）：\n"
    "   personal_info（基本信息）、summary（自我评价/个人简介）、work_experience（工作经历）、\n"
    "   education（教育经历）、projects（项目经历）、skills（技能）、certifications（资格证书）、\n"
    "   languages（语言能力）、github（GitHub 项目）、qr_codes（二维码）、custom（其他未归类）；\n"
    "3. 绝对禁止虚构、推断或补全任何原文不存在的事实（经历、量化指标、奖项、日期、GPA 等）；"
    "缺失项直接留空或省略，不要编造；\n"
    "4. 每个区块的 items 为该区块下的要点文本列表（字符串数组）；\n"
    "5. 不输出任何解释，只输出结构化结果。"
)
