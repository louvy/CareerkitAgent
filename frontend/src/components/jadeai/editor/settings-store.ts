...
function persistEditorSettings(settings: { autoSave: boolean; autoSaveInterval: number }) {
  if (typeof window === 'undefined') return;
  try {
    localStorage.setItem('jade_editor_settings', JSON.stringify(settings));
  } catch { /* ignore */ }
}
...
// Sync settings to server (debounced) — 本项目无此接口，保留占位逻辑仅更新本地
let syncTimeout: ReturnType<typeof setTimeout> | null = null;

function syncToServer(_state: SettingsStore) {
  // 本项目未提供 /api/user/settings 服务端同步，仅保留签名兼容
...
export const useSettingsStore = create<SettingsStore>((set, get) => ({
  ...
  setAutoSave: (enabled) => {
    ...
    persistEditorSettings({ autoSave: enabled, autoSaveInterval: get().autoSaveInterval });
  },

  setAutoSaveInterval: (interval) => {
    set({ autoSaveInterval: interval });
    persistEditorSettings({ autoSave: get().autoSave, autoSaveInterval: interval });
  },
  ...
  hydrate: async () => {
    ...
    // 本项目无服务端设置接口，仅从 localStorage 恢复
    try {
      const raw = localStorage.getItem('jade_editor_settings');
      if (raw) {
